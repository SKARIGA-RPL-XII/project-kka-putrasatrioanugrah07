...
<head>
    <meta charset="UTF-8">
    <title>Perusahaan - Penmedia Loker</title>
...
<body>
    <nav class="navbar">
        ...
        <div class="nav-links">
            ...
            <a href="{{ route('user.companies') }}" class="active">Perusahaan</a>
    ...
    <header class="hero">
        <div class="hero-content">
            <h1>Jelajahi Perusahaan Terbaik</h1>
            <p>Temukan perusahaan impian Anda dan pelajari budaya kerja, benefit, serta peluang karir yang tersedia</p>
        </div>
    </header>

    <main class="content">
        <div class="filters">
            <form action="{{ route('user.companies') }}" method="GET" class="filter-grid">
                <div class="filter-group">
                    <label>Cari Perusahaan</label>
                    <input type="text" name="search" placeholder="Nama perusahaan atau industri..." value="{{ request('search') }}">
                </div>
                <div class="filter-group">
                    ...
                    <select name="industry">
                        ...
                        <option value="Keuangan" {{ request('industry') == 'Keuangan' ? 'selected' : '' }}>Keuangan</option>
                ...
                <div class="filter-group">
                    ...
                    <select name="size">
                        ...
                        <option value="1-50" {{ request('size') == '1-50' ? 'selected' : '' }}>Startup (1-50)</option>
                        <option value="51-200" {{ request('size') == '51-200' ? 'selected' : '' }}>Menengah (51-200)</option>
                        <option value="200+" {{ request('size') == '200+' ? 'selected' : '' }}>Besar (200+)</option>
                    </select>
                </div>
        ...
        @if(request()->hasAny(['search', 'industry', 'size']))
        <div style="background: #eff6ff; border: 1px solid #bfdbfe; padding: 16px 20px; border-radius: 12px; margin-bottom: 24px; display: flex; justify-content: space-between; align-items: center;">
            <span style="color: #1e40af; font-weight: 600;">
                <i class='bx bx-filter-alt'></i> Filter aktif: {{ $companies->count() }} perusahaan ditemukan
            </span>
            <a href="{{ route('user.companies') }}" style="color: #2563eb; text-decoration: none; font-weight: 600; font-size: 14px;">
        ...
        <div class="companies-grid">
            ...
            <div class="company-card">
                <div class="company-header">
                    ...
                    @auth
                    <button onclick="toggleFavorite({{ $company->id }}, this)" class="favorite-btn" data-company-id="{{ $company->id }}" style="background: #e5e7eb; color: #64748b; border: none; padding: 12px; border-radius: 50%; cursor: pointer; font-size: 18px; margin-left: auto; width: 44px; height: 44px; display: flex; align-items: center; justify-content: center; transition: all 0.3s;">🤍</button>
                    @endauth
                </div>
            ...
            <div style="grid-column: 1 / -1; text-align: center; padding: 80px 20px; color: #64748b;">
                <i class='bx bxs-buildings' style="font-size: 64px; margin-bottom: 24px; opacity: 0.5;"></i>
                <h3 style="margin: 0 0 12px; font-size: 24px; font-weight: 700;">Belum Ada Perusahaan</h3>
                <p style="margin: 0; font-size: 16px;">Perusahaan akan muncul di sini setelah mendaftar</p>
            </div>
            @endforelse
        </div>
    </main>
    ...
    <script>
        ...
        document.addEventListener('DOMContentLoaded', function() {
            ...
            favoriteButtons.forEach(button => {
                ...
                fetch(`/favorites/check?company_id=${companyId}`, {
                    headers: {
                        'Accept': 'application/json',
                        'X-CSRF-TOKEN': '{{ csrf_token() }}'
                    }
